...
export default function SalesSimulatorPage() {
  ...
  const chartData = useMemo(() => {
    return simulatedResults.map(r => ({
      ...
      'Baseline Sales': Math.round(r.baseSales / 1_000_000), // in Millions
    }));
  }, [simulatedResults]);
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Target vs Simulation Composed Chart (2/3) */}
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 lg:col-span-2">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">Actual Target vs Baseline & Simulated Performance (in IDR Millions)</p>
                </div>
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 text-[10px] font-bold text-slate-400">
                  <span className="flex items-center gap-1.5">
                    <span className="w-3 h-2.5 rounded-sm bg-gradient-to-t from-blue-600 to-blue-400 inline-block" /> 
                    Simulated Sales
                  </span>
                  <span className="flex items-center gap-1.5">
                    <span className="w-3 h-0.5 bg-[#c5a880] inline-block relative after:content-[''] after:absolute after:left-1/2 after:-top-0.5 after:-translate-x-1/2 after:w-1.5 after:h-1.5 after:rounded-full after:bg-[#c5a880]" /> 
                    Target
                  </span>
                  <span className="flex items-center gap-1.5">
              ...
              <div className="h-[260px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={chartData} margin={{ top: 10, right: 8, left: 0, bottom: 0 }}>
                    <defs>
                      <linearGradient id="simSalesGrad" x1="0" y1="0" x2="0" y2="1">
                    ...
                    {/* Simulated Sales Bar */}
                    <Bar dataKey="Simulated Net Sales" name="Simulated Sales" fill="url(#simSalesGrad)" radius={[4, 4, 0, 0]} maxBarSize={28} />
                    
                    {/* Baseline Sales Line (Dashed) */}
                    <Line
                      type="monotone"
                      dataKey="Baseline Sales"
                      name="Baseline Sales"
                      stroke="#94a3b8"
                      strokeWidth={2}
                      strokeDasharray="4 4"
                    ...
                    <Line
                      ...
                      stroke="#c5a880"
                      strokeWidth={3}
                      dot={{ r: 5, fill: '#c5a880', stroke: '#fff', strokeWidth: 2 }}
                      activeDot={{ r: 7 }}
                    />
                  </ComposedChart>
                </ResponsiveContainer>
